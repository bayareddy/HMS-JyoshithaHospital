...
export function NewPatientModal({ isOpen, onClose, onSave }: ModalProps) {
  const [formData, setFormData] = useState({
    ...
    emergency: '', allergy: '', insurance: '', policy: ''
  });
        ...
        <div className="p-4 sm:p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            ...
            <div>
              ...
              <input className="w-full p-2 border border-border-subtle rounded-md text-[12px] bg-surface2 focus:bg-white focus:border-accent outline-none" placeholder="e.g. Star Health" value={formData.insurance} onChange={e => setFormData({...formData, insurance: e.target.value})} />
            </div>
          </div>
        </div>
